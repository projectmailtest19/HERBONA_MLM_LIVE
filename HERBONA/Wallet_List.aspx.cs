...
{
    public partial class Wallet_List : System.Web.UI.Page
    {
        ...
        static List<WalletlistModel> _gstlistModel = new List<WalletlistModel>();
        ...
        public static Hashtable AllLoadDetails(Hashtable ht, string Type, string Req)
        {
            try
            {
                ...
                if (Type == "Fill")
                {
                    ...
                    ds = db.SysFetchDataInDataSet("[GET_WALLET]", ht_param);
                    if (ds.Tables.Count > 0)
                    {
                        _gstlistModel = ds.Tables[0].AsEnumerable()
                          .Select(row => new WalletlistModel
                          {
                              ID = row["ID"].ToString(),
                              MEMBER_ID = row["MEMBER_ID"].ToString(),
                              WALLET_NUMBER = row["WALLET_NUMBER"].ToString(),
                ...
                if (Type == "setStatus")
                {
                    ...
                    ht_param.Add("@Company_ID", HttpContext.Current.Session["Company_ID"].ToString());
                    ht_param.Add("@Branch_ID", HttpContext.Current.Session["Branch_ID"].ToString());
                    ds = db.SysFetchDataInDataSet("[SAVE_UPDATE_WALLET]", ht_param);
                    if (ds.Tables.Count > 0)
                    {
