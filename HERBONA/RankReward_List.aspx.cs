...
{
    public partial class RankReward_List : System.Web.UI.Page
    {
        ...
        static List<RankRewardlistModel> _gstlistModel = new List<RankRewardlistModel>();
                ...
                if (Type == "Fill")
                {
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        _gstlistModel = ds.Tables[0].AsEnumerable()
                          .Select(row => new RankRewardlistModel
                          {
                              ID = row["ID"].ToString(),
