...
{
    public partial class ItemCategory_List : System.Web.UI.Page
    {
        ...
        static List<ItemCategorylistModel> _gstlistModel = new List<ItemCategorylistModel>();
                ...
                if (Type == "Fill")
                {
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        _gstlistModel = ds.Tables[0].AsEnumerable()
                          .Select(row => new ItemCategorylistModel
                          {
                              ID = row["ID"].ToString(),
