...
{
    public partial class CreateRankReward : System.Web.UI.Page
    {
        ...
        static List<RankRewardModel> _gstModel = new List<RankRewardModel>();
        ...
        public static Hashtable TaxDetails(Hashtable ht, string Type, string Req)
        {
            try
            {
                ...
                if (Type == "Fill")
                {
                    ...
                    for (int m = 0; words.Count() > m; m++)
                    {
                        ...
                        if (Data == "Edit")
                        {
                            ...
                            ds = db.SysFetchDataInDataSet("[GET_RANK_REWARD]", ht_param);

                            if (ds.Tables.Count > 0)
                            {
                                _gstModel = ds.Tables[0].AsEnumerable()
                                  .Select(row => new RankRewardModel
                                  {
                                      ID = row["ID"].ToString(),
                                      NAME = row["NAME"].ToString(),
                                      START = row["START"].ToString(),
                                      LEFT_SIDE = row["LEFT_SIDE"].ToString(),
                                      RIGHT_SIDE = row["RIGHT_SIDE"].ToString(),
                                      REWARDS_DETAIL = row["REWARDS_DETAIL"].ToString(),
                ...
                if (Type == "Save")
                {
                    ...
                    ht_param.Add("@START", ht["START"].ToString());
                    ht_param.Add("@LEFT_SIDE", ht["LEFT_SIDE"].ToString());
                    ht_param.Add("@RIGHT_SIDE", ht["RIGHT_SIDE"].ToString());
                    ht_param.Add("@REWARDS_DETAIL", ht["REWARDS_DETAIL"].ToString());
                    ht_param.Add("@IsActive", ht["IsActive"].ToString());
                    ht_param.Add("@Company_ID", HttpContext.Current.Session["Company_ID"].ToString());
                    ht_param.Add("@Branch_ID", HttpContext.Current.Session["Branch_ID"].ToString());
                    ht_param.Add("@login_id", HttpContext.Current.Session["Login_user_ID"].ToString());
                    ds = db.SysFetchDataInDataSet("[SAVE_UPDATE_RANK_REWARD]", ht_param);
                    if (ds.Tables.Count > 0)
                    {
