...
{
    public partial class CreateSessionList : System.Web.UI.Page
    {
        ...
        static List<SessionModel> _SessionModel = new List<SessionModel>();
        ...
        public static Hashtable AllLoadDetails(Hashtable ht, string Type, string Req)
        {
            try
            {
                ...
                if (Type == "Fill")
                {
                    ...
                    ds = db.SysFetchDataInDataSet("[get_Session]", ht_param);
                    if (ds.Tables.Count > 0)
                    {
                        _SessionModel = ds.Tables[0].AsEnumerable()
                          .Select(row => new SessionModel
                          {
                              ID = row["ID"].ToString(),
                              Session_Name = row["Session_Name"].ToString(),
                              from_date = row["from_date"].ToString(),
                              to_date = row["to_date"].ToString(),
                              IsActive = row["IsActive"].ToString()
                          }).ToList();
                    }
                    ReturnData["LoadData"] = serializer.Serialize(_SessionModel);
                ...
                if (Type == "setStatus")
                {
                    ...
                    ht_param.Add("@Company_ID", HttpContext.Current.Session["Company_ID"].ToString());
                    ht_param.Add("@Branch_ID", HttpContext.Current.Session["Branch_ID"].ToString());
                    ds = db.SysFetchDataInDataSet("[SAVE_UPDATE_Session]", ht_param);
                    if (ds.Tables.Count > 0)
                    {
