...
{
    public partial class ShippingCharges_List : System.Web.UI.Page
    {
        ...
        static List<ShippingChargeslistModel> _gstlistModel = new List<ShippingChargeslistModel>();
                ...
                if (Type == "Fill")
                {
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        _gstlistModel = ds.Tables[0].AsEnumerable()
                          .Select(row => new ShippingChargeslistModel
                          {
                              ID = row["ID"].ToString(),
